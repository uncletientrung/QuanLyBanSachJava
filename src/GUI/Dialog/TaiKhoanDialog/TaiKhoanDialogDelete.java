 ...
 * @author Hi
 */
public class TaiKhoanDialogDelete extends JDialog{
    private JButton btnXoa, btnHuy;
    private boolean xacNhan = false;
    private String maNv;
    private TaiKhoanDTO taiKhoanHienTai; // Thêm biến này để lưu nhóm quyền

    public TaiKhoanDialogDelete(Frame parent, TaiKhoanDTO taiKhoan) {
        ...
        // ======= Tiêu đề =======
        JLabel titleLabel = new JLabel("XÓA TÀI KHOẢN", SwingConstants.CENTER);
        ...
        JLabel lblMessage = new JLabel("Bạn có chắc chắn muốn xóa tài khoản này?", SwingConstants.CENTER);
        lblMessage.setFont(new Font("Arial", Font.BOLD, 16));
        lblMessage.setForeground(Color.DARK_GRAY);

        JLabel lblTenTaiKhoan = new JLabel(taiKhoan.getUsername(), SwingConstants.CENTER);
        lblTenTaiKhoan.setFont(new Font("Arial", Font.BOLD, 18));
        ...
        gbc.gridy = 1;
        pn_content.add(lblTenTaiKhoan, gbc);
    ...
    public TaiKhoanDTO getTaiKhoan() {
        return taiKhoanHienTai; // Trả về đối tượng nhóm quyền hiện tại
    }

    public void setController(TaiKhoanDialogDelete_Controller controller) {
