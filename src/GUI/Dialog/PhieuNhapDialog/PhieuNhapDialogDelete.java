 ...
 * @author Minnie
 */
public class PhieuNhapDialogDelete extends JDialog{
    private PhieuNhapDTO pnDTO;
    public PhieuNhapDialogDelete(JFrame parent, PhieuNhapDTO pnDTO) {
        super(parent,"Danh mục xóa phiếu nhập",true);
        this.pnDTO=pnDTO;
        ...
        // Câu hỏi xác nhận
        JLabel questionLabel = new JLabel("Bạn có chắc chắn muốn xóa phiếu nhập này?");
        ...
        // Mã phiếu
        JPanel maPhieuPanel = createInfoPanel("Mã phiếu",  pnDTO.getMaphieu()+"");

        // Tổng giá trị
        JPanel tongGiaTriPanel = createInfoPanel("Tổng giá trị", pnDTO.getTongTien()+"");
        ...
        contentPanel.add(Box.createRigidArea(new Dimension(0, 20))); // Khoảng cách giữa câu hỏi và thông tin
        contentPanel.add(Box.createVerticalGlue()); // Đẩy nội dung lên trên cùng
        ...
        // Thêm Action
        ActionListener action=new PhieuNhapDialogDelete_Controller(this);
