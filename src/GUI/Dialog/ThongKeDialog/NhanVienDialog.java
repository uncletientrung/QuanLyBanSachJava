...
public class NhanVienDialog extends JDialog {
    ...
    private DefaultTableModel tablenv2;
    private DefaultTableModel tablenv3;
    ...
    public ArrayList<NhanVienDTO> listnv = new NhanVienBUS().getNVAll();
    ...
    public NhanVienDialog(JFrame parent) {
        ...
        lbTitle.setBorder(BorderFactory.createEmptyBorder(10, 0, 15, 0)); // Tạo nvoảng cách dưới tiêu đề
        headerPanel.add(lbTitle, BorderLayout.NORTH);

        JPanel searchPanel = new JPanel();
         // Tạo nvoảng cách giữa tìm kiếm và tiêu đề
        ...
        //tìm kiem nvi dang go hoac an nut
        ...
        //bảng dữ liệu
        String[] columnNCC = {"Mã", "Họ", "Tên"};
        tablenv = new DefaultTableModel(columnNCC, 0) {
            ;
        ...
        for (NhanVienDTO ncc : listnv) {
            tablenv.addRow(new Object[]{ncc.getManv(), ncc.getHonv(), ncc.getTennv()});
        ...
        // table1
        String[] columnNCC2 = {"Mã", "Họ", "Tên","Số hóa đơn","Doanh thu","Tổng sách"};
        tablenv2 = new DefaultTableModel(columnNCC2, 0) {
            ;
        ...
        //ham nay de cho nvong focus con tro vao o tim kiem
    ...
    private void timKiemKhiDangGo() {
        ...
        //nvi ấn tìm mới kiểm tra tìm thấy, tìm nvông thấy mới thông báo
    ...
    public void capNhatBang(ArrayList<NhanVienDTO> danhSach) {
        tablenv.setRowCount(0); // Xóa bảng cũ

        for (NhanVienDTO ncc : danhSach) {
            tablenv.addRow(new Object[]{ncc.getManv(), ncc.getHonv(), ncc.getTennv(), ncc.getNgaysinh(), ncc.getSdt()});
        }
    }

    //load lai du lieu nvi moi them vo
    public void loadData() {
        listnv = new NhanVienBUS().getNVAll(); // Lấy danh sách mới
        capNhatBang(listnv); // Cập nhật lại bảng
    }
    ...
    public NhaCungCapDTO getSelectedNhaCungCap() {
        ...
        if (selectedRow == -1) {
            return null; // Nếu nvông chọn gì, trả về null
        }
        ...
        JButton button = new JButton(text) {
            ...
            protected void paintComponent(Graphics g) {
                ...
                if (getModel().isPressed()) {
                    actualBgColor = bgColor.darker(); // Màu tối hơn nvi nhấn
                } else if (getModel().isRollover()) {
                    actualBgColor = bgColor.brighter(); // Màu sáng hơn nvi hover
                }
    ...
    public void suatable( ArrayList<NhanVienDTO> listnv , ArrayList<Long> list ){
        int n=0;
         for (NhanVienDTO ncc : listnv) {
             
            tablenv2.addRow(new Object[]{ncc.getManv(), ncc.getHonv(), ncc.getTennv(), list.get(n),list.get(n+1),list.get(n+2)});
                n+=3;
        }
    }
    
    public void suatable2( ArrayList<NhanVienDTO> listnv  ){
