...
public class NhanVienDialog extends JDialog {
    ...
    private ArrayList<NhanVienDTO> listnv = new NhanVienBUS().getNVAll();
    ...
    public NhanVienDialog(JFrame parent) {
        super(parent, "Thống kê nvách hàng", true);
        setSize(1300, 700);
        setResizable(false);
        ...
        JLabel lbTitle = new JLabel("Thông kê nvách hàng", JLabel.CENTER);
        ...
        //tim kiem
        txtTimKiem.addFocusListener(new NhanVienController(this, workFrame));
        //tìm kiem nvi dang go hoac an nut
        txtTimKiem.getDocument().addDocumentListener(new NhanVienController(this, workFrame));
        ...
        header.addMouseListener(new NhanVienController(this, workFrame));
    ...
    public void capNhatBang(ArrayList<NhanVienDTO> danhSach) { 
        ...
        for (int i=0;i<danhSach.size();i++) {
            for (int j=0;j<list.size();j++)
                if(String.valueOf(danhSach.get(i).getManv()).equals(String.valueOf(list.get(j)[1]))){
                    tablenv.addRow(list.get(j));
    ...
    public  ArrayList <Object[]> List(){
        ...
        for(int i=0;i<listnv.size();i++){
            if(list!=null){
                list1.add(new Object[]{i+1,listnv.get(i).getManv(),listnv.get(i).getHonv()+" "+listnv.get(i).getTennv(),listthongke.get(t),listthongke.get(t+1),listthongke.get(t+2),listthongke.get(t+3)});
                t+=4;
            }else 
                list1.add(new Object[]{i+1,listnv.get(i).getManv(),listnv.get(i).getHonv()+" "+listnv.get(i).getTennv(),-1,-1,-1,-1});
        }
    ...
    public DefaultTableModel getTablenv() {
        return tablenv;
    }
